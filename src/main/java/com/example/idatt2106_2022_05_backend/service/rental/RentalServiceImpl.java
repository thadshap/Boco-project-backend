...
public class RentalServiceImpl implements RentalService {
    ...
    public Response createRental(RentalDto rentalDto) throws MessagingException, IOException {
        ...
        if (adFound.isPresent()) {
            ...
            Rental rental = Rental.builder().borrower(borrower).owner(owner).ad(ad)
            ...
            Email email = Email.builder().from("BOCO@gmail.com").to(rental.getOwner().getEmail())
                    .template(new ThymeleafTemplate("verify_maillon", variables)).subject("Låneforespørsel hos BOCO")
                    .build();
    ...
    public Response activateRental(Long rentalId) throws MessagingException, IOException {
        ...
        Email email = Email.builder().from("BOCO@gmail.com").to(rental.getBorrower().getEmail())
                .template(new ThymeleafTemplate("verify_maillon", variables))
                .subject("Låneforespørsel hos BOCO er godtatt").build();
        emailService.sendEmail(email);
    ...
    public Response declineRental(Long rentalId) throws MessagingException, IOException {
        ...
        Email email = Email.builder().from("BOCO@gmail.com").to(rental.getBorrower().getEmail())
                .template(new ThymeleafTemplate("verify_maillon", variables))
                .subject("Låneforespørsel hos BOCO er avslått").build();
        emailService.sendEmail(email);
    ...
    public Response getRental(Long rentalId) {
        ...
        Rental rental = rentalOptional.get();
        RentalDto rentalReturn = RentalDto.builder().id(rental.getId()).adId(rental.getAd().getId())
                ...
                .active(rental.isActive()).dateOfRental(rental.getDateOfRental()).deadline(rental.getDeadline())
                .rentFrom(rental.getRentFrom()).rentTo(rental.getRentTo()).price(rental.getPrice()).build();
    ...
    public Response getRentalsByUserId(Long userId) {
        ...
        for (int i = 0; i < rental.size(); i++) {
            RentalDto rentalReturn = RentalDto.builder().id(rental.get(i).getId()).adId(rental.get(i).getAd().getId())
                    .title(rental.get(i).getAd().getTitle())
                    .borrower(rental.get(i).getBorrower().getFirstName() + " "
                            + rental.get(i).getBorrower().getLastName())
                    .owner(rental.get(i).getOwner().getFirstName() + " " + rental.get(i).getOwner().getLastName())
                    .active(rental.get(i).isActive()).dateOfRental(rental.get(i).getDateOfRental())
                    .deadline(rental.get(i).getDeadline()).rentFrom(rental.get(i).getRentFrom())
                    .rentTo(rental.get(i).getRentTo()).price(rental.get(i).getPrice())
                    .isReviewed(rental.get(i).isReviewed()).build();
    ...
    public Response getRentalPictureById(Long rentalId) {
        ...
        if (pictures.isEmpty()) {
            return new Response("No pictures found for rental with ad title \"" + rental.getAd().getTitle() + "\"",
                    HttpStatus.NO_CONTENT);
        }
        PictureReturnDto returnDto = PictureReturnDto.builder()
                // .base64(Base64.getEncoder().encodeToString(pictures.get(0).getData()))
                .base64(pictures.get(0).getBase64()).type(pictures.get(0).getType()).build();
