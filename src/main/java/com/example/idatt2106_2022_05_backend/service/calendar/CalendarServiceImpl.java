...
public class CalendarServiceImpl implements CalendarService {
        ...
        if (adFound.isPresent()) {
            ...
            for (CalendarDate date : datesBelongingToAd) {
                ...
                if (date1.isEqual(startDate)
                        || (date1.isAfter(startDate) && date1.isBefore(endDate) || date1.isEqual(endDate))) {
     ...
     *
     * @param dto
     *            contains: the start date the end date (boolean) available --> for use as mutator
     * 
        ...
        if (rental.isPresent()) {
            ...
            if (dto.isAvailable()) {
                ...
                // If 24 hrs has passed, the cancellation cannot be made
                    ...
                    return new Response("More than 24hrs after rental has passed", HttpStatus.NOT_ACCEPTABLE);
                }
            }
            // isAvailable cannot be false when a rental has not yet been made
            else {
                return new Response("isAvailable cannot be false when a rental is present", HttpStatus.NOT_ACCEPTABLE);
            }
        }
        ...
        if (ad.isPresent()) {
            ...
            for (CalendarDate date : dates) {
                ...
                if (theDate.isEqual(dto.getStartDate())
                     ...
                     * 
                     * if (dateFound.isPresent()) { dateFound.get().setAvailable(dto.isAvailable());
                     * dateRepository.save(dateFound.get()); } else { // If the date found is not present something very
                     * wrong happened :p return new Response(null, HttpStatus.NOT_FOUND); }
                     */
                }
                // }
            }
    ...
    public Set<CalendarDate> addFutureDates(long adId) {
        ...
        if (ad.isPresent()) {
            ...
            for (int i = 1; i < 366; i++) {
                ...
                CalendarDate newDate = CalendarDate.builder().available(true).date(creationDate.plusDays(i)).build();
